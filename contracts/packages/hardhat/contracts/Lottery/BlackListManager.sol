...
contract BlackListManager is AccessControl {
    ...
    function addToBlackList(address _player) external onlyRole(OPERATOR_ROLE) {
        BlackListStruct memory blElement = BlackListStruct({blacklisted: true, dateBlacklisted: block.timestamp});
    ...
    function removeFromBlackList(address _player) external onlyRole(OPERATOR_ROLE) {
