...
const GasLessBuyFighter: FC<IGasLessBuyFighter> = ({ battleId }) => {
  ...
  const { data: balance }: { data: any } = useScaffoldContractRead("FundManager", "getBalance", [address]);
  ...
  const { writeAsync, isLoading } = useScaffoldContractWrite("ChainWarzLottery", "createEntries", args);

  const gaslessEntry = async () => {
    try {
      setLoading(true);
      // toast.loading("Mining Tx. Hold on");
      ...
      let afterBalance;
      // toast.remove("Mining Tx. Hold on");
      ...
      if (txReceipt.status === 1) {
        ...
        // console.log(`Gas Cost: ${+beforeBalance - +afterBalance}`);
      } else {
        toast.error("Tx Failed!");
      }
    } catch (e) {
      toast.error(`${getParsedEthersError(e)}. Args: ${args}`);
    } finally {
      setLoading(false);
    }
  };
