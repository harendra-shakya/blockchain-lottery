...
export default async (req: NextApiRequest, res: NextApiResponse) => {
    if (req.method === "POST") {
        ...
        if (!(price && network && deployedContractData && amountOfEntries && address && nativeCurrencyPrice)) {
        ...
        try {
            ...
            console.log("args", battleId, amountOfEntries, address!, +price);
            ...
            logger.info(`Waiting for conformation..... ${getBlockExplorerTxLink(_network, tx.hash)}`);
            ...
            if (txReceipt.status === 1) {
                ...
                logger.info(
                    ...
                    Gas : ${+ethers.utils.formatEther(
                        (+txReceipt.cumulativeGasUsed * +txReceipt.effectiveGasPrice).toString(),
                    )} ETH.
                    Dollars: ${
