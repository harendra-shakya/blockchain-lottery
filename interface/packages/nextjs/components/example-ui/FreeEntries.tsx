...
export default function FreeEntries() {
    ...
    const giveBatchEntriesForFree = () => {
        if (signature && address) {
            ...
            notification.error(`args missing ${args}`);
            if (!address) {
                notification.error(`address is ${address}`);
            }
            if (!signature) {
                notification.error(`signature is ${signature}`);
            }
        }
    };
    ...
    const args: (boolean | number | string | string[])[] = [address!, 0, signature!];
