...
const Notification = ({
    ...
    return toast.custom(
        t => (
            <div
                className={`primary-gradient-r p-[1.7px] rounded-3xl transform-gpu relative transition-all duration-300 ease-in-out shadow-white shadow-2xl hover:shadow-none  ${
            ...
            >
                <div
                    className={`font-orbitron flex flex-row items-start justify-between max-w-md rounded-3xl bg-white p-4 space-x-2
       `}
                >
                    <div className="text-2xl self-start">{icon ? icon : ENUM_STATUSES[status]}</div>
                    <div className={`text-black break-all whitespace-pre-line ${icon ? "mt-1" : ""}`}>{content}</div>
